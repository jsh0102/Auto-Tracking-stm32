...
> | 캡처→송신 지연 | — ms |
> | 전체 end-to-end 지연 | — ms |
> | 정착 시간 (오차 300px → ±20px) | — s |
> | 정상상태 오차 | — px |
>
> 추론 시간은 `tracker.py`가 이미 콘솔에 출력합니다(`[T2] infer=...ms`).
> end-to-end는 STM32에서 패킷 수신 시 GPIO를 토글해 로직 애널라이저로 측정합니다.
